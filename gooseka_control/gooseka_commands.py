...
class GoosekaCommands(Commands):
    ...
    def _limit_with_mppt(self, telemetry, target_left, target_right):
        """ Duty limitations with MPPT

        Keyword arguments:
        telemetry -- dict with telemetry information
        target_left -- target duty for the left motor
        target_right -- target duty for the right motor

        """

        logger.info("Executing MPTT")
        # FIXME mppt duty should be modified here
        total_duty = self.mptt.step(
            telemetry, self.duty_left + self.duty_right)
    ...
    def _filter_duty_slew_rate(self, duty, past_duty):
        ...
        if "MOT_SLEWRATE" in self.config and self.config["MOT_SLEWRATE"] > 0:
            if abs(duty - past_duty) > (
                    ...
                    (1.0/self.config["MOT_SLEWRATE"]) *
                    self.config["LOOP_CONTROL_MS"]/1000.0):
    ...
    def _execute_loop_control(self, telemetry, execute_mppt):
        """ Execute the loop control 

        Keyword arguments:
        telemetry -- dict with telemetry information
        execute_mppt -- true if mppt should be calculated
        ...
        if (current_millis - self.last_control_ms >
            ...
            if "left" in telemetry and "right" in telemetry:
                ...
                # Limit with MPPT
                if execute_mppt:
                    target_left, target_right = self._limit_with_mppt(telemetry, target_left, target_right)
    ...
    def get_command(self, telemetry):
        ...
        self._execute_loop_control(telemetry, execute_mppt=mppt_flag)
    ...
    def __init__(self, config):
        ...
        self.axis_steer = self.config["STEER"]
